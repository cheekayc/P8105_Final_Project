...
A pie chart showing the percentage of each product’s sale count (top 10) in 2021
```{r}
plot_pie =
  bakery_df %>% 
...
plot_line =
  ...
  filter(product_name %in% c("TRADITIONAL BAGUETTE", "COUPE", "BAGUETTE","BANETTE", "CROISSANT", "PAIN", "AU CHOCOLAT", "SPECIAL BREAD", "CEREAL BAGUETTE", "FORMULE SANDWICH", "BOULE 400G")) %>% 
  mutate(product_name = fct_reorder(product_name, prod_rev)) %>% 
  ggplot(aes(x =product_name, y= prod_rev, fill=product_name)) + 
    geom_bar(stat="identity", width=0.5) +
    labs(x = "name of products",
         y = "total revenue",
         title = "ranking of total revenue of top 10 products") +
